        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
                                ...
                                if (nbClients==4)
				{
					...
					// Rappel: Le joueur 0 possede les cartes d'indice 0,1,2 //ajouté↓
					...
					// Rappel: // Le joueur 1 possede les cartes d'indice 3,4,5 //ajouté↓
					...
					// Rappel: // Le joueur 2 possede les cartes d'indice 6,7,8 //ajouté↓
					...
					// Rappel: // Le joueur 3 possede les cartes d'indice 9,10,11 //ajouté↓
					...
					// On envoie enfin un message a tout le monde pour definir qui est le joueur courant=0 //ajouté↓
					sprintf(reply, "M %d", joueurCourant);
					broadcastMessage(reply);
                                        fsmServer=1;
				}
		...
		else if (fsmServer==1)
		{
			switch (buffer[0])
			{
				...
				break;
					...
					printf("Joueur %d interroge joueur %d à propos de l'Objet #%d\n", id, joueurSel, objetSel);
					sprintf(reply, "R %d %d %d", joueurSel, objetSel, tableCartes[joueurSel][objetSel]);
					sendMessageToClient(tcpClients[id].ipAddress, tcpClients[id].port, reply);
					updateTurn();
				break;
                	default:
