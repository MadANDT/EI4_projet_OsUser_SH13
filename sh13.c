...
volatile int synchro; // `volatile` permet de ne pas accéder à la valeur qu'il y a dans le cache mais plutôt la vraie valeur (à la bonne adresse)
...
int main(int argc, char ** argv)
{
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	if (SDL_PollEvent(&event))
	{
		...
		switch (gbuffer[0])
		{
			// Message 'I' : le joueur recoit son Id //ajouté↓
			case 'I':
				sscanf(gbuffer, "I %d", &gId);
				printf("Received ID: %d\n", gId);
				break;

			// Message 'L' : le joueur recoit la liste des joueurs //ajouté↓
			case 'L':
			...
			// Message 'D' : le joueur recoit ses trois cartes//ajouté↓
			case 'D':
				sscanf(gbuffer, "D %d %d %d", &b[0], &b[1], &b[2]);
				printf("Received cards: %d, %d, %d\n", b[0], b[1], b[2]);
				break;
			
			// Message 'M' : le joueur recoit le n° du joueur courant //ajouté↓
			...
			// Message 'V' : le joueur recoit une valeur de tableCartes //ajouté↓
			case 'V':
				int value;
			...
			// Cas des réponses du serveur //ajouté↓
			case 'R':
				int count;
